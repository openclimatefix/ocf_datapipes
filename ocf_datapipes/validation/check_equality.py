...
class CheckGreaterThanOrEqualToIterDataPipe(IterDataPipe):
    def __init__(self, source_datapipe: IterDataPipe, min_value: int, dataset_name: Optional[str] = None):
    ...
    def __iter__(self):
        for xr_data in self.source_datapipe:
            if self.dataset_name is not None:
                check_dataset_greater_than_or_equal_to(xr_data[self.dataset_name], min_value=self.min_value)
            else:
...
class CheckLessThanOrEqualToIterDataPipe(IterDataPipe):
    def __init__(self, source_datapipe: IterDataPipe, max_value: int, dataset_name: Optional[str] = None):
        self.source_datapipe = source_datapipe
    ...
    def __iter__(self):
        for xr_data in self.source_datapipe:
            if self.dataset_name is not None:
                check_dataset_less_than_or_equal_to(xr_data[self.dataset_name], max_value=self.max_value)
            else:
...
class CheckNotEqualToIterDataPipe(IterDataPipe):
    def __init__(self, source_datapipe: IterDataPipe, value: int, dataset_name: Optional[str] = None, raise_error: bool = true):
        self.source_datapipe = source_datapipe
        self.value = value
        self.dataset_name = dataset_name
    ...
    def __iter__(self):
        for xr_data in self.source_datapipe:
            if self.dataset_name is not None:
                check_dataset_not_equal(xr_data[self.dataset_name], value=self.value, raise_error=self.raise_error)
            else:
...
def check_dataset_not_equal(
        data: xr.Dataset, value: int, raise_error: bool = True
):
