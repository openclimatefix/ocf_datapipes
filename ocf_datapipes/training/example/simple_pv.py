...
from ocf_datapipes.load import load_configuration, OpenPVFromDB, OpenPVFromNetCDF
...
def simple_pv_datapipe(
    ...
    pv_datapipe, pv_t0_datapipe, pv_time_periods_datapipe, pv_location_datapipe = (
        pv_datapipe.fork(4, buffer_size=BUFFERSIZE)
    )

    # Preprocess PV timeseries
    pv_datapipe = (
        pv_datapipe.normalize(normalize_fn=normalize_pv)
    )
