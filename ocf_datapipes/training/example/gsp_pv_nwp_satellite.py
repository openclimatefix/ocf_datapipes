...
from ocf_datapipes.load import load_configuration, OpenGSP, OpenNWP, OpenPVFromNetCDF, OpenSatellite
...
def gsp_pv_nwp_satellite_data_pipeline(
    ...
    satellite_datapipe = (
        OpenSatellite(zarr_path=configuration.input_data.satellite.satellite_zarr_path)
    ...
    for nwp_source in nwp_datapipe_dict.keys():
        nwp_datapipe_dict[nwp_source], nwp_time_periods_datapipes[nwp_source] = (
            nwp_datapipe_dict[nwp_source].add_t0_idx_and_sample_period_duration(
    ...
    gsp_datapipe, gsp_time_periods_datapipe, gsp_valid_times_datapipe, gsp_location_datapipe = (
        gsp_datapipe.fork(4)
    )
    ...
    for nwp_source, nwp_conf in configuration.input_data.nwp.items():
        ...
        nwp_datapipe_dict[nwp_source] = (
            nwp_datapipe_dict[nwp_source].select_spatial_slice_pixels(
                loc_dp,
