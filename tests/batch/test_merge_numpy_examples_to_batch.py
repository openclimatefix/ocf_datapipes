...
def _single_batch_sample(fill_value):
    ...
    sample: NumpyBatch = {}
    sample[BatchKey.satellite_actual] = np.full((12, 10, 24, 24), fill_value) # shape: (time, channel, x, y)
    ...
    sample_nwp_ukv[NWPBatchKey.nwp] = np.full((8, 2, 24, 24), fill_value) # shape: (time, variable, x, y) 
...
def test_merge_numpy_batch(numpy_sample_datapipe):
    ...
    for i in range(2):
        batch = next(dp_iter)
        assert (batch[BatchKey.satellite_actual][:, 0,0,0,0] == np.arange(4*i, 4*(i+1))).all()
