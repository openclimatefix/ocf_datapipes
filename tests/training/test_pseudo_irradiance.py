...
def test_irradiance_datapipe():
    ...
    gsp_datapipe = pseudo_irradiance_datapipe(
        filename, use_nwp=True, use_sat=True, use_hrv=True, use_sun=True, use_future=False, size=16
    )
...
def test_irradiance_datapipe_public_data():
    ...
    gsp_datapipe = pseudo_irradiance_datapipe(
        filename, use_nwp=True, use_sat=True, use_hrv=True, use_sun=False, use_future=False, size=16
    )
    batch = next(iter(gsp_datapipe))
    batch = (torch.Tensor(batch[0]), torch.Tensor(batch[1]), torch.Tensor(batch[2]))
    x = np.nan_to_num(batch[0])
